...
class MyForm(forms.Form):
    team_choices = [("ALL", "All Teams")] + [(team.id, team.name) for team in TeamIds.objects.all()]
    team = forms.ChoiceField(choices=team_choices)
    min_pa = forms.ChoiceField(choices=[(x, x) for x in range(0, 126, 25)])
    season_choices = [("ALL", "All Seasons")] + [(x.season, x.season) for x in LeagueOffenseOverall.objects.order_by('-season')]
